...
def main():
    ...
    with open(args.jsonfile, 'r') as jsonfile:
        ...
        init_frame = pd.read_table(file_name, sep='\t', compression='gzip', header=None, names=['ensemble_id', submitter_id])
        ...
        for sample_next_info in jsonfile_iter:
            submitter_id, file_name = sample_next_info['associated_entities'][0]['entity_submitter_id'].strip(), sample_next_info[
                'file_name'].strip()
            sample_next = pd.read_table(file_name, sep='\t', compression='gzip', header=None, names=['ensemble_id', submitter_id])
            init_frame = pd.merge(init_frame, sample_next, on=['ensemble_id'])
